...
| 6 | `backend/models/courses.py` → `students.py` → `attendance.py` → `assessments.py` → `stats.py` | The domain rules: ownership, roll uniqueness, schedule clashes, the 75% and missing=0 rules |
...
backend/
  ...
  models/             ALL SQL lives here (parameterized, nothing else)
    ...
    courses.py        courses, sections, schedule-clash check, enrollment
...
6. **`courses._ensure_no_clash()`** — interval-overlap SQL
